---
title: Reflect
excerpt: Reflect对象与`Proxy`对象一样，也是ES6为了操作对象而提供的新API。
toc: true
tag: 'JavaScript'
categories:
- 前端
...
`Reflect`对象与`Proxy`对象一样，也是ES6为了操作对象而提供的新API。`Reflect`对象设计的目的有这样几个
...
`Reflect.get`方法查找并返回`target`对象的`name`属性的值，如果没有该属性，则返回`undefined`。
